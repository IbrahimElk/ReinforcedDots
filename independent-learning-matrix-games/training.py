...
def get_all_red_lines(
    ...
    for i in range(amt_distinct_start_probs):
        ...
        history_of_probs = train_agents(agent1, agent2, game=game, training_episodes=100000)
        ...
        case "Q":
            ...
            agents =[
                QLearner(player_id=idx, 
                        ...
                        epsilon_schedule=rl_tools.LinearSchedule(0.5, 0.05, 100000),
                        step_size=step_size
                        )
        ...
        case "B" : 
            ...
            agents =[
                BoltzmannQLearner(player_id=idx, 
                                num_actions=num_actions,
                                step_size = step_size,
                                temperature_schedule=rl_tools.LinearSchedule(0.5, 0.01, 101000))
                for idx in range(num_players)
            ]
        case "LB" : 
            step_size = 0.001
            agents =[
                LenientBoltzmannQLearner(player_id=idx, 
                                num_actions=num_actions,
                                temperature_schedule=rl_tools.LinearSchedule(0.5, 0.01, 100000),
                                kappa=kappa,
                                step_size=step_size)
                for idx in range(num_players)
            ]
